...
const TagSelector = ({ 
  ...
  // Convert registry to array for dropdown
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                color: '#666', fontSize: '14px', lineHeight: 1, padding: 0,
...
export default function AdminContentManager() {
  ...
  const [editHall, setEditHall] = useState({ name: "", slug: "", imageUrl: "", description: "" });
  ...
  const saveMenuItemEdits = async () => {
    ...
    try {
      await updateMenuItem(editItemId, {
        ...
        tags: editItemForm.tags, // ✅ Send ID array directly
        macros: {
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.tabRow}>
        <button className={`${styles.tabBtn} ${tab === "addHall" ? styles.tabActive : ""}`} onClick={() => setTab("addHall")} type="button">Add Hall</button>
        <button className={`${styles.tabBtn} ${tab === "manageHalls" ? styles.tabActive : ""}`} onClick={() => setTab("manageHalls")} type="button">Manage Halls</button>
        <button className={`${styles.tabBtn} ${tab === "menuItems" ? styles.tabActive : ""}`} onClick={() => setTab("menuItems")} type="button">Menu Items</button>
      </div>

      {/* ADD HALL / MANAGE HALLS SECTIONS (Keep Same) */}
      {tab === "addHall" && (
         <div className={styles.panel}>
           {/* ... (Existing Add Hall Code) ... */}
           <h3 className={styles.panelTitle}>Add Dining Hall</h3>
            ...
            <div className={styles.actionsRow}>
              <button className={styles.primaryBtn} type="button" disabled={saving} onClick={createHall}>{saving ? "Saving..." : "Create Dining Hall"}</button>
            </div>
         </div>
      )}
      ...
      {tab === "manageHalls" && (
         <div className={styles.panel}>
            ...
            {!selectedHall ? <div className={styles.muted}>Please select a dining hall above.</div> : (
              <>
                <div className={styles.grid2}>
                  <div className={styles.field}><label className={styles.label}>Name</label><input className={styles.input} value={editHall.name} onChange={(e) => setEditHall(p => ({...p, name: e.target.value}))} /></div>
                  <div className={styles.field}><label className={styles.label}>Slug</label><input className={styles.input} value={editHall.slug} onChange={(e) => setEditHall(p => ({...p, slug: e.target.value}))} /></div>
                </div>
                <div className={styles.field}><label className={styles.label}>Image URL</label><input className={styles.input} value={editHall.imageUrl} onChange={(e) => setEditHall(p => ({...p, imageUrl: e.target.value}))} /></div>
                <div className={styles.field}><label className={styles.label}>Description</label><textarea className={styles.textarea} rows={3} value={editHall.description} onChange={(e) => setEditHall(p => ({...p, description: e.target.value}))} /></div>
                <div className={styles.actionsRow}>
                   <button className={styles.dangerBtn} type="button" disabled={saving} onClick={removeHall}>Delete Hall</button>
                   <button className={styles.primaryBtn} type="button" disabled={saving} onClick={saveHall}>{saving ? "Saving..." : "Save Changes"}</button>
                </div>
              </>
            )}
         </div>
        ...
        <div className={styles.panel}>
          ...
          {drafts.map((d, idx) => (
            <div key={idx} className={styles.itemRow}>
              ...
              <div className={styles.field}>
                ...
                <TagSelector 
                  selected={d.tags} 
                  onChange={(newTags) => updateDraft(idx, { tags: newTags })} 
                />
              </div>
              ...
              <div style={{display:'flex', justifyContent:'flex-end'}}>
                 <button className={styles.iconBtn} type="button" onClick={() => removeDraftRow(idx)} disabled={drafts.length === 1}>Delete Row ✕</button>
              </div>
            </div>
          ))}

          <div className={styles.actionsRow}>
            <button className={styles.ghostBtn} type="button" onClick={addDraftRow}>+ Add Another Row</button>
            <button className={styles.primaryBtn} type="button" disabled={saving || !itemsHallSlug} onClick={saveBatchMenuItems}>{saving ? "Saving..." : "Upload Batch"}</button>
          </div>
          ...
          {!itemsHallSlug ? (
            ...
            <>
              ...
              {!editItemId ? <div className={styles.muted}>Pick a menu item to edit details.</div> : (
                <div style={{ marginTop: '20px', borderTop: '1px solid var(--color-border-soft)', paddingTop: '20px' }}>
                  ...
                  <div className={styles.field}>
                    ...
                    <TagSelector 
                        selected={editItemForm.tags} 
                        onChange={(newTags) => setEditItemForm(p => ({ ...p, tags: newTags }))} 
                    />
                  </div>
